<template>
  <main class="framing-main">
    ...
    <form class="framing-form museum-label" @submit.prevent>
      ...
      <width-slider label="Frame" v-model="config.frameWidth" :min="20" :max="50"></width-slider>
      <frame-style-picker v-model="config.frameStyle"></frame-style-picker>
      <width-slider label="Mat" :min="0" :max="100" v-model="config.matWidth"></width-slider>
      <mat-color-picker v-model="config.matColor"></mat-color-picker>
      <fieldset>
        <legend>Price</legend>
...
export default {
  ...
  methods: {
    addToCart: function () {
      ...
      this.$store.dispatch("addToCart", product).then((ok) => {
        if (ok) {
